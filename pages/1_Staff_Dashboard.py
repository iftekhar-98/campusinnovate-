...
<style>
  ...
  .stApp {
    background: linear-gradient(135deg, #0d1a3a 0%, #11224e 50%, #162a5e 100%);
    min-height: 100vh;
  }

  /* Dashboard header — aurora gradient */
  .dash-header {
    background: linear-gradient(135deg, #1B306B 0%, #243d82 60%, #F47920 100%);
  ...
  .kpi-purple { background: linear-gradient(135deg, rgba(27,48,107,0.85), rgba(36,61,130,0.7)); }
  .kpi-red    { background: linear-gradient(135deg, rgba(220,38,38,0.7),  rgba(248,113,113,0.5)); }
  .kpi-orange { background: linear-gradient(135deg, rgba(244,121,32,0.85), rgba(255,160,60,0.6)); }
  ...
  .badge-blue   { background:rgba(27,48,107,0.40);   color:#FFF; border:1px solid rgba(27,48,107,0.6); padding:3px 10px; border-radius:999px; font-size:12px; font-weight:600; }
  ...
  div[data-testid="stButton"] > button[kind="primary"] {
    background: linear-gradient(135deg, #1B306B, #F47920);
    border: 1px solid rgba(255,255,255,0.2);
...
<div class="dash-header">
  ...
  <img src="data:image/png;base64,{logo_base64}" style="height:52px;width:auto;" alt="CampusInnovate Logo">
    <div>
      <h1 style="margin:0;font-size:22px;color:white">UCI Operations Dashboard</h1>
      <p style="margin:4px 0 0;font-size:13px;opacity:.8;color:rgba(255,255,255,0.8)">CampusInnovate — AI-Assisted Signal Governance</p>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
...
with tab_analytics:
    ...
    with chart_col1:
        ...
        if cats:
            ...
            fig = px.pie(df_cat, names="category", values="count", hole=0.55,
                         color_discrete_sequence=["#1B306B","#F47920","#34D399","#FBBF24","#FB7185","#60A5FA","#243d82"])
            fig.update_layout(
    ...
    if daily:
        ...
        fig3 = px.area(df_daily, x="date", y="count",
                       color_discrete_sequence=["#F47920"])
        fig3.update_layout(
        ...
        fig3.update_traces(line_width=2.5, fillcolor="rgba(244,121,32,0.15)")
